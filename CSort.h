...
template <typename T> struct SortCompareVar {
  int (*greaterThan)(T, T, struct SortCompareVar<T>);
  void *data;
};

template <class T> class CSort {
...
public:
  ...
  static void quicksort(std::vector<T> *array, int left_begin, int right_begin,
                        struct SortCompareVar<T> sortCompare);
  static void quicksort(std::vector<T *> *array, int left_begin,
                        int right_begin);
  static int binarySearchInOrdered(std::vector<T> *array, T toFind);
  static void insertInOrdered(std::vector<T> *array, T toInsert);
  static void insertInOrdered(std::vector<T> *array, T toInsert,
                              struct SortCompareVar<T> sortCompare);
  // static void CSort::quicksort(std::vector array[], int left_begin, int
  // right_begin);
};

template <class T> CSort<T>::CSort() {}

template <class T> CSort<T>::~CSort() {}

template <class T>
void CSort<T>::quicksort(std::vector<T> *array, int left_begin,
                         int right_begin) {
...
void CSort<T>::quicksort(std::vector<T> *array, int left_begin, int right_begin,
  ...
  do {
    // WINFO("looop");
    while ((sortCompare.greaterThan)((*array)[left_index], pivot,
                                     sortCompare) == -1 &&
           left_index < right_begin)
      left_index++;
    while ((sortCompare.greaterThan)((*array)[right_index], pivot,
                                     sortCompare) == 1 &&
           right_index > left_begin)
      right_index--;
...
void CSort<T>::insertInOrdered(std::vector<T> *array, T toInsert,
  ...
  while (Lbound <= Ubound) {
    ...
    if ((sortCompare.greaterThan)(toInsert, (*array)[Mid], sortCompare) == 1) {
      ...
      positionToInsert = Lbound;
      // WVARIABLE(Lbound);
    } else if ((sortCompare.greaterThan)(toInsert, (*array)[Mid],
                                         sortCompare) == -1) {
      Ubound = Mid - 1;
      positionToInsert = Ubound;
      // WVARIABLE(Ubound);
