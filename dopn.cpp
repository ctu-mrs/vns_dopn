...
// >DOPN::allDistancesUsage;

DOPN::DOPN(GraphNode start, GraphNode end, double radius, int resolution,
           double neighborhood_radius, int neighborhood_resolution,
           bool null_start_goal_radius_)
    : start(start), end(end), radius(radius), resolution(resolution),
      neighborhood_radius(neighborhood_radius),
      neighborhood_resolution(neighborhood_resolution),
      null_start_goal_radius(null_start_goal_radius_) {
...
void DOPN::calcAllDistances(std::vector<GraphNode> allGraphNodes, double radius,
                            int resolution, double neighborhood_radius,
  ...
  for (int var = 0; var < S; ++var) {
    generateSamples(allSamples[var], allGraphNodes[var], resolution,
                    neighborhood_radius, neighborhood_resolution,
                    null_start_goal_radius, startIndex, goalIndex);
  }
  ...
  for (int gnid1 = 0; gnid1 < allGraphNodes.size(); ++gnid1) {
    ...
    for (int gnid2 = 0; gnid2 < allGraphNodes.size(); ++gnid2) {
      ...
      if (null_start_goal_radius &&
          (gnid1 == startIndex || gnid1 == goalIndex)) {
        maxneighID1 = 1;
      }
      ...
      for (int neighID1 = 0; neighID1 < maxneighID1; ++neighID1) {
        ...
        if (null_start_goal_radius &&
            (gnid2 == startIndex || gnid2 == goalIndex)) {
          maxneighID2 = 1;
        }
        ...
        for (int neighID2 = 0; neighID2 < maxneighID2; ++neighID2) {
          ...
          for (int idx1 = 0; idx1 < resolution; ++idx1) {
            allDistances[gnid1][gnid2][neighID1][neighID2][idx1].resize(
                resolution);
  ...
  for (int gnid1 = 0; gnid1 < allGraphNodes.size(); ++gnid1) {
    ...
    for (int gnid2 = 0; gnid2 < allGraphNodes.size(); ++gnid2) {
      ...
      for (int neighID1 = 0; neighID1 < allSamples[gn1Id].size(); ++neighID1) {
        for (int neighID2 = 0; neighID2 < allSamples[gn2Id].size();
             ++neighID2) {
          for (int idx1 = 0; idx1 < allSamples[gn1Id][neighID1].size();
               ++idx1) {
            // allDistances[gnid1][gnid2][idx1].resize(resolution);
            for (int idx2 = 0; idx2 < allSamples[gn2Id][neighID2].size();
                 ++idx2) {
              ...
              // INFO("calc dubins from "<<from.toState()<<" to
              // "<<to.toState());
              Dubins dub(from.toState(), to.toState(), radius);
              // INFO("save length");
              allDistances[gn1Id][gn2Id][neighID1][neighID2][idx1][idx2] =
                  dub.length;
              // allDistancesUsage[gnid1][gnid2][neighID1][neighID2][idx1][idx2]
              // = 0;
              numNodes++;
              // allDubins[gn1.id][gn2.id][neighID1][neighID2][idx1][idx2] =
              // dub;
            }
          }
        }
...
std::vector<std::vector<std::vector<GraphNode_AngNeigh>>> *
DOPN::getAllSamples() {
  return &allSamples;
}

std::vector<GraphNode> DOPN::getAllTargets() { return targets; }
        ...
        for (int neighID2 = 0; neighID2 < allSamples[gn2Id].size();
          ...
          for (int idx1 = 0; idx1 < allSamples[gn1Id][neighID1].size();
            ...
            for (int idx2 = 0; idx2 < allSamples[gn2Id][neighID2].size();
              ...
              if (allDistances[gn1Id][gn2Id][neighID1][neighID2][idx1][idx2] !=
                  euclideanDistance) {
      ...
      for (int neighID1 = 0; neighID1 < allSamples[gn1Id].size(); ++neighID1) {
        for (int neighID2 = 0; neighID2 < allSamples[gn2Id].size();
          ...
          for (int idx1 = 0; idx1 < allSamples[gn1Id][neighID1].size();
            ...
            for (int idx2 = 0; idx2 < allSamples[gn2Id][neighID2].size();
                 ++idx2) {
              if (allSamples[gn1Id].size() !=
                  allDistances[gn1Id][gn2Id].size()) {
                ERROR("sizes does not match - 1");
                exit(1);
              }
              if (allSamples[gn1Id][neighID1].size() !=
                  allDistances[gn1Id][gn2Id][neighID1][neighID2].size()) {
              ...
              GraphNode_AngNeigh &to = allSamples[gn2Id][neighID2][idx2];
              Dubins dub(from.toState(), to.toState(), radius);
              if (gn1Id >= allDistances.size() ||
                  gn2Id >= allDistances[gn1Id].size() ||
                  neighID1 >= allDistances[gn1Id][gn2Id].size() ||
                  neighID2 >= allDistances[gn1Id][gn2Id][neighID1].size() ||
                  idx1 >=
                      allDistances[gn1Id][gn2Id][neighID1][neighID2].size() ||
                  idx2 >= allDistances[gn1Id][gn2Id][neighID1][neighID2][idx1]
                              .size() ||
                  fabs(allDistances[gn1Id][gn2Id][neighID1][neighID2][idx1]
                                   [idx2] -
                       dub.length) > 0.01) {
...
void DOPN::generateSamples(
    std::vector<std::vector<GraphNode_AngNeigh>> &samples, GraphNode p,
    double resolution, double neighborhood_radius, int neighborhood_resolution,
...
int DOPN::getSize() const { return targets.size() + 1; }

void DOPN::addPoint(GraphNode p, int idx) {
  // INFO("addPoint single begin");
  if (idx > targets.size() || idx < 0) {
    INFO_RED("addPoint idx " << idx << " exceeds maximum "
                             << (targets.size() - 1));
    exit(1);
  }

  // INFO("add point "<<p.id<<" between "<<samples[idx][0][0].node.id<<" and
  // "<<samples[idx+1][0][0].node.id); double lengthBef = getPathLength();
  targets.insert(targets.begin() + idx, p);
  // INFO("inserted point "<<p.id <<" into "<<idx<<" position , target size
  // "<<targets.size())
...
void DOPN::removePoint(int idx) {
  ...
  // INFO("removePoint id "<<samples[idx+1][0][0].node.id<<" that is between
  // "<<samples[idx][0][0].node.id<<" and "<<samples[idx+2][0][0].node.id);
  if (idx >= targets.size() || idx < 0) {
    INFO_RED("getTarget idx " << idx << " exceeds maximum "
                              << (targets.size() - 1));
    exit(1);
  }
  double lengthBef = getPathLength();
...
double DOPN::optimizeHeading(int targetId,
                             double minimal_improvement_distance) {
  int inSampleId = targetId + 1;
  std::vector<std::vector<GraphNode_AngNeigh>> &sample =
      this->samples.at(inSampleId);
  ...
  GraphNode_AngNeigh angNeighPrev =
      samples[inSampleId - 1][neighIdPrev][angIdPrev];

  int neighIdNext = pathNeightIds[inSampleId + 1].idNeigh;
  int angIdNext = pathNeightIds[inSampleId + 1].idAng;
  GraphNode_AngNeigh angNeighNext =
      samples[inSampleId + 1][neighIdNext][angIdNext];
  ...
  double actual_ang =
      normalizeAngle(sample[neighId][angId].ang, ang_min, ang_min + M_2PI);
  double ang_max =
      normalizeAngle(sample[neighId][(angId + 1) % sample[neighId].size()].ang,
                     ang_min, ang_min + M_2PI);
  ...
  while (angleChange > 0.01) {
    ...
    double distanceAfter_1 = angDistanceAfter(editingPosState, ang_to_set_1,
                                              previousState, nextState);
    double distanceAfter_2 = angDistanceAfter(editingPosState, ang_to_set_2,
                                              previousState, nextState);
    ...
    if (distanceAfter_1 < iterativeDistance ||
        distanceAfter_2 < iterativeDistance) {
  ...
  if (fabs(actual_ang - iterativeActualAngle) > 0.05 &&
      actualDistance - iterativeDistance > 0.01) {
...
void DOPN::insertAngSample(int inSampleId, int neighId, int angIdActual,
                           int angIdWhereInsert,
                           GraphNode_AngNeigh editing_pos) {
  ...
  allSamples[nodeId][neighId].insert(
      allSamples[nodeId][neighId].begin() + angIdWhereInsert, angSampleOld);
  allSamples[nodeId][neighId][angIdWhereInsert].ang = editing_pos.ang;

  // copy distances -- insert from old
  for (int otherNodeId = 0; otherNodeId < allDistances[nodeId].size();
       ++otherNodeId) {
    for (int neighID2 = 0;
         neighID2 < allDistances[nodeId][otherNodeId][neighId].size();
         ++neighID2) {
      std::vector<double> distancesAngles =
          allDistances[nodeId][otherNodeId][neighId][neighID2][angIdActual];
      allDistances[nodeId][otherNodeId][neighId][neighID2].insert(
          allDistances[nodeId][otherNodeId][neighId][neighID2].begin() +
              angIdWhereInsert,
          distancesAngles);
    }

    for (int otherNeighId = 0;
         otherNeighId < allDistances[otherNodeId][nodeId].size();
         ++otherNeighId) {
      for (int otherAngId = 0;
           otherAngId <
           allDistances[otherNodeId][nodeId][otherNeighId][neighId].size();
           ++otherAngId) {
        double distancesAngle = allDistances[otherNodeId][nodeId][otherNeighId]
                                            [neighId][otherAngId][angIdActual];
        allDistances[otherNodeId][nodeId][otherNeighId][neighId][otherAngId]
            .insert(allDistances[otherNodeId][nodeId][otherNeighId][neighId]
                                [otherAngId]
                                    .begin() +
                        angIdWhereInsert,
                    distancesAngle);
  ...
  for (int otherNodeId = 0; otherNodeId < allDistances[nodeId].size();
       ++otherNodeId) {
    // calc distances inside
    // allDistances[nodeId][otherNodeId][neighIdWhereInsert] that was inserted
    // before
    for (int neighID2 = 0;
         neighID2 < allDistances[nodeId][otherNodeId][neighId].size();
         ++neighID2) {
      for (int idx2 = 0; idx2 < allDistances[nodeId][otherNodeId][neighId]
                                            [neighID2][angIdWhereInsert]
                                                .size();
           ++idx2) {
        GraphNode_AngNeigh &from =
            allSamples[nodeId][neighId][angIdWhereInsert];
        GraphNode_AngNeigh &to = allSamples[otherNodeId][neighID2][idx2];
        Dubins dub(from.toState(), to.toState(), radius);
        allDistances[nodeId][otherNodeId][neighId][neighID2][angIdWhereInsert]
                    [idx2] = dub.length;
      }
    }
    ...
    for (int otherNeighId = 0;
         ...
         ++otherNeighId) {
      // calc distances inside
      // allDistances[otherNodeId][nodeId][otherNeighId][neighIdWhereInsert]
      ...
      for (int idx1 = 0;
        ...
        Dubins dub(from.toState(), to.toState(), radius);
        allDistances[otherNodeId][nodeId][otherNeighId][neighId][idx1]
                    [angIdWhereInsert] = dub.length;
      }
    }
  }
...
NeighImprovement DOPN::optimizeNeighborhoodPosition(
  ...
  if (targetId - 1 >= 0) {
    ...
    previousState.point.x =
        previousTargetCenterNode.x +
        this->neighborhood_radius * cos(actualNeighAngles[targetId - 1]);
    previousState.point.y =
        previousTargetCenterNode.y +
        this->neighborhood_radius * sin(actualNeighAngles[targetId - 1]);
  } else {
  ...
  if (targetId + 1 < targets.size()) {
    ...
    nextState.point.x =
        nextTargetCenterNode.x +
        this->neighborhood_radius * cos(actualNeighAngles[targetId + 1]);
    nextState.point.y =
        nextTargetCenterNode.y +
        this->neighborhood_radius * sin(actualNeighAngles[targetId + 1]);
  } else {
  ...
  editing_pos.node.x =
      targetCenterNode.x +
      this->neighborhood_radius * cos(actualNeighAngles[targetId]);
  editing_pos.node.y =
      targetCenterNode.y +
      this->neighborhood_radius * sin(actualNeighAngles[targetId]);
  ...
  double neigh_ang_max =
      normalizeAngle(sample[(neighId + 1) % sample.size()][0].neigh_ang,
                     neigh_ang_min, neigh_ang_min + M_2PI);
  double actual_neigh_ang = normalizeAngle(
      actualNeighAngles[targetId], neigh_ang_min, neigh_ang_min + M_2PI);
  ...
  while (angleChange >
         0.02) { // && lastIterativeDistance - iterativeDistance > 0.005) {

    double ang_to_set_1 = iterativeActualAngle + angleChange;
    double ang_to_set_2 = iterativeActualAngle - angleChange;
    double distanceAfter_1 = neighborhoodAngDistanceAfter(
        targetCenterNode, editingState, ang_to_set_1, previousState, nextState);
    double distanceAfter_2 = neighborhoodAngDistanceAfter(
        targetCenterNode, editingState, ang_to_set_2, previousState, nextState);
  ...
  if (fabs(actual_neigh_ang - iterativeActualAngle) > 0.2 &&
      actualDistance - iterativeDistance > 0.005) {
    // if ( actualDistance - iterativeDistance > minimal_improvement_distance) {
    // if (iterativeDistance != actualDistance) {
    editing_pos.neigh_ang = normalizeAngle(iterativeActualAngle, 0, M_2PI);
    editing_pos.node.x = targetCenterNode.x +
                         this->neighborhood_radius * cos(editing_pos.neigh_ang);
    editing_pos.node.y = targetCenterNode.y +
                         this->neighborhood_radius * sin(editing_pos.neigh_ang);
    State editingPosStateAfter = editing_pos.toState();
    Dubins dubPrev(previousState, editingPosStateAfter, radius);
    Dubins dubNext(editingPosStateAfter, nextState, radius);
    double actualDistanceAfter = dubPrev.length + dubNext.length;
    ...
    /*
     ...
     //insert before neighId for target in stample - at position inSampleId
     insertNeighSample(inSampleId, neighId, neighId, editing_pos.neigh_ang,
     editing_pos.node); } else {
     //insert after neighId
     insertNeighSample(inSampleId, neighId, neighId + 1, editing_pos.neigh_ang,
     editing_pos.node);
     }
     */
    // INFO_CYAN("improved");
...
void DOPN::insertNeighSample(int inSampleId, int neighIdActual,
                             int neighIdWhereInsert, double newAngle,
                             GraphNode node) {
  int nodeId = node.id;
  // INFO("insertNeighSample to nodeId "<<nodeId);
  std::vector<GraphNode_AngNeigh> angSamplesOld =
      allSamples[nodeId][neighIdActual];

  // samples[inSampleId].insert(samples[inSampleId].begin() +
  // neighIdWhereInsert, angSamplesOld);
  allSamples[nodeId].insert(allSamples[nodeId].begin() + neighIdWhereInsert,
                            angSamplesOld);
  // INFO("add new samples");
  for (int ang_i = 0; ang_i < allSamples[nodeId][neighIdWhereInsert].size();
       ang_i++) {
  ...
  for (int otherNodeId = 0; otherNodeId < allDistances[nodeId].size();
    ...
    std::vector<std::vector<std::vector<double>>> distances =
        allDistances[nodeId][otherNodeId][neighIdActual];
    allDistances[nodeId][otherNodeId].insert(
        allDistances[nodeId][otherNodeId].begin() + neighIdWhereInsert,
        distances);
    for (int otherNeighId = 0;
         otherNeighId < allDistances[otherNodeId][nodeId].size();
         ++otherNeighId) {
      std::vector<std::vector<double>> distancesAngles =
          allDistances[otherNodeId][nodeId][otherNeighId][neighIdActual];
      allDistances[otherNodeId][nodeId][otherNeighId].insert(
          allDistances[otherNodeId][nodeId][otherNeighId].begin() +
              neighIdWhereInsert,
          distancesAngles);
    }
  }
  ...
  for (int otherNodeId = 0; otherNodeId < allDistances[nodeId].size();
    ...
    for (int neighID2 = 0;
         neighID2 <
         allDistances[nodeId][otherNodeId][neighIdWhereInsert].size();
         ++neighID2) {
      for (int idx1 = 0;
           idx1 <
           allDistances[nodeId][otherNodeId][neighIdWhereInsert][neighID2]
               .size();
           ++idx1) {
        for (int idx2 = 0;
             idx2 < allDistances[nodeId][otherNodeId][neighIdWhereInsert]
                                [neighID2][idx1]
                                    .size();
             ++idx2) {
          GraphNode_AngNeigh &from =
              allSamples[nodeId][neighIdWhereInsert][idx1];
          GraphNode_AngNeigh &to = allSamples[otherNodeId][neighID2][idx2];
          Dubins dub(from.toState(), to.toState(), radius);
          allDistances[nodeId][otherNodeId][neighIdWhereInsert][neighID2][idx1]
                      [idx2] = dub.length;
        }
      }
    }
    ...
    for (int otherNeighId = 0;
      ...
      for (int idx1 = 0;
           ...
           ++idx1) {
        for (int idx2 = 0;
             idx2 < allDistances[otherNodeId][nodeId][otherNeighId]
                                [neighIdWhereInsert][idx1]
                                    .size();
             ++idx2) {
          GraphNode_AngNeigh &from =
              allSamples[otherNodeId][otherNeighId][idx1];
          GraphNode_AngNeigh &to = allSamples[nodeId][neighIdWhereInsert][idx2];
          Dubins dub(from.toState(), to.toState(), radius);
          allDistances[otherNodeId][nodeId][otherNeighId][neighIdWhereInsert]
                      [idx1][idx2] = dub.length;
        }
      }
    }
  }
...
double DOPN::neighborhoodAngDistanceAfter(GraphNode centroid,
  ...
  State newState =
      State(new_x, new_y,
            editingState
                .ang); // chagne only the x and y, keep the ang of editing state
...
double DOPN::angDistanceAfter(State editingState, double new_ang,
                              State previousState, State nextState) {
  State newState =
      State(editingState.point.x, editingState.point.y,
            new_ang); // chagne only the x and y, keep the ang of editing state
  Dubins dubPrevNew(previousState, newState, radius);
  Dubins dubNextNew(newState, nextState, radius);
  double distanceAfter = dubPrevNew.length + dubNextNew.length;
  return distanceAfter;
...
SimpleTryResult DOPN::tryToAdd(GraphNode p, int idx) const {
  ...
  if (idx > targets.size() || idx < 0) {
    INFO_RED("tryToAdd idx " << idx << " exceeds maximum "
                             << (targets.size() - 1));
    exit(1);
  }
  ...
  // INFO("tryToAdd "<< p.id <<" between "<<samples[idx][0][0].node.id<<" and
  // "<<samples[idx+1][0][0].node.id);
    ...
    for (int toNeighID = 0; toNeighID < actSamples.size(); ++toNeighID) {
      for (int idx1 = 0; idx1 < fromSamples[fromNeighID].size(); idx1++) {
        for (int idx2 = 0; idx2 < actSamples[toNeighID].size(); idx2++) {
          ...
          double dl = allDistances[from.node.id][to.node.id][fromNeighID]
                                  [toNeighID][idx1][idx2];
      ...
      for (int idx1 = 0; idx1 < actSamples[fromNeighID].size(); idx1++) {
        for (int idx2 = 0; idx2 < toSamples[toNeighID].size(); idx2++) {
          ...
          double dl = allDistances[from.node.id][to.node.id][fromNeighID]
                                  [toNeighID][idx1][idx2];
          double nl =
              dl + toLen[toNeighID][idx2].distance + actLen[fromNeighID][idx1];
          if (nl < minLen) {
            minLen = nl;
          }

          // double nl_max = dl + toLen[toNeighID][idx2].distance +
          // actLen[fromNeighID][idx1]; INFO_VAR(nl_max);
          if (nl > maxLen) {
            // INFO_VAR(maxLen);
            maxLen = nl;
          }
...
double DOPN::tryToAdd(std::vector<GraphNode> newPoints, int idx) const {
  ...
  // generate samples
  std::vector<std::vector<std::vector<GraphNode_AngNeigh>>> actSamples(
      addingNumPoints);
  ...
  for (int target = 1; target < addingNumPoints + 2; target++) {
    shortestadded[target].resize(
        allDistances[fromNodeId][newPoints[target - 1].id].size());
    for (int neighID = 0;
         neighID < allDistances[fromNodeId][newPoints[target - 1].id].size();
         ++neighID) {
      shortestadded[target][neighID].assign(resolution,
                                            NeighAngDist(-1, -1, M));
    }

    for (int neighID1 = 0;
         neighID1 < allDistances[fromNodeId][newPoints[target - 1].id].size();
         ++neighID1) {
      for (int neighID2 = 0;
           neighID2 <
           allDistances[fromNodeId][newPoints[target - 1].id][neighID1].size();
           ++neighID2) {

        for (int idx1 = 0; idx1 < resolution; idx1++) {
          for (int idx2 = 0; idx2 < resolution; idx2++) {
            auto len1 = shortestadded[target - 1][neighID1][idx1].distance;
            auto len2 = allDistances[fromNodeId][newPoints[target - 1].id]
                                    [neighID1][neighID2][idx1][idx2];
            // INFO(idx1<<" "<<idx2<<" len "<<len1<<" dist "<<distances[target -
            // 1][idx1][idx2]);
            auto nLen = len1 + len2;
  ...
  double minLen = M;
  for (int neighID2 = 0; neighID2 < shortestadded[addingNumPoints + 1].size();
       neighID2++) {
    for (int idx2 = 0; idx2 < toSamples.size(); idx2++) {
      double nl = shortestadded[addingNumPoints + 1][neighID2][idx2].distance +
                  toLen[neighID2][idx2].distance;
      if (nl < minLen) {
        minLen = nl;
      }
    }
...
double DOPN::tryToReplace(std::vector<GraphNode> p, int idx) const {
  ...
  for (int target = 1; target < addingNumPoints + 2; target++) {
    // INFO("target "<<target);
    shortestadded[target].resize(
        allDistances[fromNodeId][p[target].id][0].size());
    ...
    for (int neighID1 = 0;
         neighID1 < allDistances[fromNodeId][p[target - 1].id].size();
         ++neighID1) {
      for (int neighID2 = 0;
           neighID2 <
           allDistances[fromNodeId][p[target - 1].id][neighID1].size();
           ++neighID2) {
        ...
        for (int idx1 = 0; idx1 < resolution; idx1++) {
          for (int idx2 = 0; idx2 < resolution; idx2++) {
            ...
            auto len1 = shortestadded[target - 1][neighID1][idx1].distance;
            auto len2 = allDistances[fromNodeId][p[target - 1].id][neighID1]
                                    [neighID2][idx1][idx2];
...
SimpleTryResult DOPN::tryToReplace(GraphNode p, int idx,
                                   int numToReplace) const {
  ...
  // generate samples
  std::vector<std::vector<GraphNode_AngNeigh>> replaceWSamples =
      allSamples[p.id];
  ...
  double maxLen = -M;
  // INFO("calc from");
  std::vector<std::vector<GraphNode_AngNeigh>> toSamples =
      samples[idx + 1 + numToReplace];
  dop_2d_matrix_triple toLen = shortest_back[idx + 1 + numToReplace];
  // find the shortest path from actual samples to next <toSamples>

  for (int fromNeighID = 0; fromNeighID < replaceWSamples.size();
       ++fromNeighID) {
    for (int idx1 = 0; idx1 < replaceWSamples[fromNeighID].size(); idx1++) {
      ...
      for (int toNeighID = 0; toNeighID < toSamples.size(); ++toNeighID) {
        for (int idx2 = 0; idx2 < toSamples[toNeighID].size(); idx2++) {
          ...
          double dl = allDistances[from.node.id][to.node.id][fromNeighID]
                                  [toNeighID][idx1][idx2];
          double nl = dl + toLen[toNeighID][idx2].distance +
                      newToDistances[fromNeighID][idx1];
          if (nl < minLen) {
            minLen = nl;
          }
...
double DOPN::tryToRemove(int idx) const {
  ...
  // INFO("try to remove id "<<samples[idx+1][0][0].node.id<<" that is between
  // "<<samples[idx][0][0].node.id<<" and "<<samples[idx+2][0][0].node.id);
  double minLen = M;
...
double DOPN::tryToRemove(int idxStart, int idxStop) const {
  ...
  double minLen = M;
  for (int fromNeighID = 0; fromNeighID < shortest[idxStart].size();
       ++fromNeighID) {
    dop_vector_triple fromLen = shortest[idxStart][fromNeighID];
    std::vector<GraphNode_AngNeigh> fromSamples =
        samples[idxStart][fromNeighID];
    for (int toNeighID = 0; toNeighID < shortest_back[idxStop + 2].size();
         ++toNeighID) {
      dop_vector_triple toLen = shortest_back[idxStop + 2][toNeighID];
      std::vector<GraphNode_AngNeigh> toSamples =
          samples[idxStop + 2][toNeighID];
      for (int idx1 = 0; idx1 < fromSamples.size(); idx1++) {
        for (int idx2 = 0; idx2 < toSamples.size(); idx2++) {
          auto &from = fromSamples[idx1];
          auto &to = toSamples[idx2];
...
double DOPN::tryToExchange(int idx1, int idx2) {
  ...
  if (idx1 > targets.size() || idx1 < 0 || idx2 > targets.size() || idx2 < 0 ||
      abs(idx1 - idx2) < 2) {
    INFO_RED("tryToExchange idx " << idx1 << " or " << idx2
                                  << " exceeds maximum "
                                  << (targets.size() - 1));
    exit(1);
  }
  ...
  if (idxTo - idxFrom < 2) {
    INFO_RED("idxEnd and idxStart are too close " << (idxTo - idxFrom)
                                                  << " must be lower than 2");
    exit(1);
  }
  if (idxTo >= targets.size()) {
    INFO_RED("idxEnd+1 is out of samples array " << idxTo << " samples number "
                                                 << targets.size());
    exit(1);
  }
  std::stringstream ss;
  ...
  std::vector<std::vector<GraphNode_AngNeigh>> samples_I_p =
      samples[i + 1]; // i+1
  std::vector<std::vector<GraphNode_AngNeigh>> samples_J_m =
      samples[j - 1];                                                  // j -1
  std::vector<std::vector<GraphNode_AngNeigh>> samples_J = samples[j]; // j
  std::vector<std::vector<GraphNode_AngNeigh>> samples_J_p =
      samples[j + 1]; // j+1
  std::vector<std::vector<GraphNode_AngNeigh>> samples_I_pp =
      samples[i + 2]; // i+2
  ...
  dop_2d_matrix_triple normalOrderTotLen =
      shortest[i]; // need to use idxStart+1 as to the

  dop_2d_matrix_triple normalOrderFromLen = shortest_back[j + 1];

  // INFO("find shortest paths between reverseOrderFromSamples to
  // reverseOrderToSamples");
  // find shortest paths between reverseOrderFromSamples to
  // reverseOrderToSamples find the shortest part of path that is reversed
  // matrix:
  //    1) to city (layer)
  //    2) to idx i
  ...
  pathPartShortest[0] =
      normalOrderTotLen; // set actual shortest distance to idxStart
  ...
  // pathPartShortest[idxPathPartShortest].assign(reverseOrderFromSamples.size(),
  // dst_pair(-1, M));
  pathPartShortest[idxPathPartShortest].resize(samples_J.size());
  for (int neighID = 0; neighID < pathPartShortest[idxPathPartShortest].size();
       ++neighID) {
    pathPartShortest[idxPathPartShortest][neighID].assign(
        samples_J[neighID].size(), NeighAngDist(-1, -1, M));
  }
    ...
    for (int neighID2 = 0; neighID2 < samples_J.size(); ++neighID2) {
      for (int idx1 = 0; idx1 < samples_I[neighID1].size(); idx1++) {
        for (int idx2 = 0; idx2 < samples_J[neighID2].size(); idx2++) {
          auto len1 =
              pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                  .distance; // shortest dist from start to [target - 1][idx1]
          auto len2 = allDistances[idI][idJ][neighID1][neighID2][idx1]
                                  [idx2]; // dist from [target - 1][idx1] to
                                          // [target][idx2]
          auto nLen = len1 + len2;
          auto &len = pathPartShortest[idxPathPartShortest][neighID2]
                                      [idx2]; // actual shortest path from start
                                              // to [target][idx2]
          if (nLen < len.distance) {
            len.idNeigh = neighID1;
            len.idAng = idx1;
  ...
  for (int neighID = 0; neighID < pathPartShortest[idxPathPartShortest].size();
       ++neighID) {
    pathPartShortest[idxPathPartShortest][neighID].assign(
        samples_I_pp[neighID].size(), NeighAngDist(-1, -1, M));
  }
      ...
      for (int idx1 = 0; idx1 < samples_J[neighID1].size(); idx1++) {
        for (int idx2 = 0; idx2 < samples_I_pp[neighID2].size(); idx2++) {
          auto len1 =
              pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                  .distance; // shortest dist from start to [target - 1][idx1]
          auto len2 = allDistances[idJ][idI_pp][neighID1][neighID2][idx1]
                                  [idx2]; // dist from [target - 1][idx1] to
                                          // [target][idx2]
          auto nLen = len1 + len2;
          auto &len = pathPartShortest[idxPathPartShortest][neighID2]
  ...
  // calc from j to i+1
  for (int target = idxStartSamples + 1; target <= idxEndSamples - 2;
       target++) {
    // INFO("----------------");
    // pathPartShortest[idxPathPartShortest].assign(samples[target].size(),
    // dst_pair(-1, M));
    int idFrom = target;
    int idTo = target + 1;
    pathPartShortest[idxPathPartShortest].resize(samples[idTo].size());
    for (int neighID = 0;
         neighID < pathPartShortest[idxPathPartShortest].size(); ++neighID) {
      pathPartShortest[idxPathPartShortest][neighID].assign(
          samples[idTo][neighID].size(), NeighAngDist(-1, -1, M));
    }

    // INFO("----calc shortest from "<< samples[idFrom][0].node.id <<" to "<<
    // samples[idTo][0].node.id); INFO("idFrom " << samples[idFrom][0].node.id);
          ...
          for (int idx2 = 0; idx2 < S2; idx2++) {
            ...
            auto len1 =
                pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                    .distance; // shortest dist from start to [target - 1][idx1]
            // auto len2 =
            // distances[idTo][getInverseId(idx2)][getInverseId(idx1)]; //need
            // to use idto because of indexing of distances
            auto len2 = allDistances[from.node.id][to.node.id][neighID1]
                                    [neighID2][idx1][idx2];

            auto nLen = len1 + len2;
            // INFO(idx1<<" "<<idx2 <<" len1 "<<len1<<" len2 "<<len2<<" nLen
            // "<<nLen); INFO(idx1<<" "<<idx2<<" len "<<len1<<" dist "<<
            // distances[idTo][idx2][idx1]);
            auto &len = pathPartShortest[idxPathPartShortest][neighID2]
                                        [idx2]; // actual shortest path from
                                                // start to [target][idx2]
  ...
  for (int neighID = 0; neighID < samples_I_p.size(); ++neighID) {
    pathPartShortest[idxPathPartShortest][neighID].assign(
        samples_I_p[neighID].size(), NeighAngDist(-1, -1, M));
  }
      ...
      for (int idx1 = 0; idx1 < samples_J_m[neighID1].size(); idx1++) {
        for (int idx2 = 0; idx2 < samples_I_p[neighID2].size(); idx2++) {
          auto len1 =
              pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                  .distance; // shortest dist from start to [target - 1][idx1]
          auto len2 = allDistances[idJ_m][idI_p][neighID1][neighID2][idx1]
                                  [idx2]; // dist from [target - 1][idx1] to
                                          // [target][idx2]
          auto nLen = len1 + len2;
          auto &len = pathPartShortest[idxPathPartShortest][neighID2]
      ...
      for (int idx1 = 0; idx1 < samples_I_p[neighID1].size(); idx1++) {
        for (int idx2 = 0; idx2 < samples_J_p[neighID2].size(); idx2++) {
          auto len1 =
              pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                  .distance; // shortest dist from start to [target - 1][idx1]
          auto len2 = allDistances[idI_p][idJ_p][neighID1][neighID2][idx1]
                                  [idx2]; // dist from [target - 1][idx1] to
                                          // [target][idx2]

          // INFO("shortest "<<" len1 "<<len1 <<" len2 "<<len2<<"
          // normalOrderFromLen[idx2].second
          // "<<normalOrderFromLen[idx2].second);
...
double DOPN::tryToMoveEuclidean(int idxWhatToMove, int idxWhereToMove) {
  ...
  // double distanceAfterMove = distanceBefore - (distanceBefore - afterRemove)
  // + (afterAdd - distanceBefore);
  ...
  /*
   ...
   distanceAfterMove = distanceBefore - (distanceBefore - afterRemoveUp) +
   (afterAddUp - distanceBefore);
   }
   */
...
double DOPN::tryToMove(int idxWhatToMove, int idxWhereToMove) {
  ...
  // INFO_GREEN("tryToMove idxWhatToMove " << idxWhatToMove << " idxWhereToMove
  // " << idxWhereToMove); INFO("tryToMove getNumTargets "<<getNumTargets());
  if (idxWhatToMove >= targets.size() || idxWhatToMove < 0 ||
      idxWhereToMove > targets.size() || idxWhereToMove < 0 ||
      abs(idxWhereToMove - idxWhatToMove) < 2) {
    INFO_RED("tryToMove idxWhatToMove " << idxWhatToMove << " idxWhereToMove "
                                        << idxWhereToMove);
    INFO_RED("tryToMove getNumTargets " << getNumTargets());
    INFO_RED("tryToMove idxWhatToMove "
             << idxWhatToMove << " or idxWhereToMove " << idxWhereToMove
             << " exceeds maximum for target size" << (targets.size()));
    exit(1);
  }
  // INFO("-------------------------------------------------------------------------");
  bool moveUp = idxWhereToMove > idxWhatToMove;
  ...
  int j = idxTo + 1;
  // INFO("get samples");
  std::vector<std::vector<GraphNode_AngNeigh>> samples_i_m =
      samples[i - 1];                                                  // i-
  std::vector<std::vector<GraphNode_AngNeigh>> samples_i = samples[i]; // i
  std::vector<std::vector<GraphNode_AngNeigh>> samples_i_p =
      samples[i + 1]; // i+1
  ...
  // INFO("get shortest");
  dop_2d_matrix_triple normalOrderTotLen =
      shortest[i - 1]; // need to use idxStart+1 as to the
  ...
  if (moveUp) {
    ...
    // INFO("ommit "<<samples_i[0][0].node.id<<" that is between
    // "<<samples_i_m[0][0].node.id<<" and "<<samples_i_p[0][0].node.id);

    for (int neighID1 = 0;
         neighID1 < pathPartShortest[idxPathPartShortest - 1].size();
         ++neighID1) {
      for (int neighID2 = 0;
           neighID2 < pathPartShortest[idxPathPartShortest].size();
           ++neighID2) {
        for (int idx1 = 0; idx1 < samples_i_m[neighID1].size(); idx1++) {
          for (int idx2 = 0; idx2 < samples_i_p[neighID2].size(); idx2++) {
            auto len1 =
                pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                    .distance; // shortest dist from start to [target - 1][idx1]
            auto len2 = allDistances[idI_m][idI_p][neighID1][neighID2][idx1]
                                    [idx2]; // dist from [target - 1][idx1] to
                                            // [target][idx2]
            auto nLen = len1 + len2;
            auto &len = pathPartShortest[idxPathPartShortest][neighID2]
    ...
    for (int target = i + 1; target < j - 1; target++) {
      ...
      // INFO("a calc shortest from "<< samples[idFrom][0][0].node.id <<" to "<<
      // samples[idTo][0][0].node.id); INFO_VAR(idFrom); INFO_VAR(idTo);
        ...
        for (int neighID2 = 0; neighID2 < samples[idTo].size(); ++neighID2) {
          ...
          for (int idx1 = 0; idx1 < S1; idx1++) {
            for (int idx2 = 0; idx2 < S2; idx2++) {
              ...
              auto len1 =
                  pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                      .distance; // shortest dist from start to [target -
                                 // 1][idx1]
              //	INFO("len2");
              auto len2 = allDistances[from.node.id][to.node.id][neighID1]
                                      [neighID2][idx1][idx2];
    ...
    for (int neighID = 0; neighID < samples_i.size(); ++neighID) {
      pathPartShortest[idxPathPartShortest][neighID].assign(
          samples_i[neighID].size(), NeighAngDist(-1, -1, M));
    }
      ...
      for (int neighID2 = 0;
        ...
        for (int idx1 = 0; idx1 < samples_j_m[neighID1].size(); idx1++) {
          for (int idx2 = 0; idx2 < samples_i[neighID2].size(); idx2++) {
            ...
            auto len2 = allDistances[idJ_m][idI][neighID1][neighID2][idx1]
                                    [idx2]; // dist from [target - 1][idx1] to
                                            // [target][idx2]
            auto nLen = len1 + len2;
    ...
    // INFO("add "<<samples_i_m[0][0].node.id<<" beween
    // "<<samples_j_m[0][0].node.id<<" and "<<samples_j[0][0].node.id);
      ...
      for (int neighID2 = 0; neighID2 < normalOrderFromLen.size(); ++neighID2) {
        for (int idx1 = 0; idx1 < samples_i[neighID1].size(); idx1++) {
          for (int idx2 = 0; idx2 < samples_j[neighID2].size(); idx2++) {
            ...
            // "<<normalOrderFromLen[idx2].second);
            auto nLen =
                len1 + len2 + normalOrderFromLen[neighID2][idx2].distance;
            if (nLen < minLen) {
              minLen = nLen;
            }
          }
    ...
    for (int neighID1 = 0;
      ...
      for (int neighID2 = 0;
           ...
           ++neighID2) {
        for (int idx1 = 0;
             idx1 < pathPartShortest[idxPathPartShortest - 1][neighID1].size();
             idx1++) {
          for (int idx2 = 0;
               idx2 < pathPartShortest[idxPathPartShortest][neighID2].size();
               idx2++) {
            // INFO_VAR(neighID1);
            // INFO_VAR(neighID2);
            // INFO_VAR(idx1);
            ...
            auto len2 = allDistances[idI_m][idJ][neighID1][neighID2][idx1]
                                    [idx2]; // dist from [target - 1][idx1] to
                                            // [target][idx2]
            auto nLen = len1 + len2;
        ...
        for (int idx1 = 0;
          ...
          for (int idx2 = 0;
            ...
            auto len1 =
                pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                    .distance; // shortest dist from start to [target - 1][idx1]
            auto len2 = allDistances[idJ][idI][neighID1][neighID2][idx1]
                                    [idx2]; // dist from [target - 1][idx1] to
                                            // [target][idx2]
            auto nLen = len1 + len2;
            auto &len = pathPartShortest[idxPathPartShortest][neighID2]
    ...
    for (int target = i; target < j - 1; target++) {
      ...
      for (int neighID1 = 0;
        ...
        for (int neighID2 = 0;
          ...
          for (int idx1 = 0;
               idx1 <
               pathPartShortest[idxPathPartShortest - 1][neighID1].size();
               idx1++) {
            for (int idx2 = 0;
                 idx2 < pathPartShortest[idxPathPartShortest][neighID2].size();
                 idx2++) {
    ...
    for (int neighID1 = 0;
      ...
      for (int neighID2 = 0; neighID2 < normalOrderFromLen.size(); ++neighID2) {
        for (int idx1 = 0;
             idx1 < pathPartShortest[idxPathPartShortest - 1][neighID1].size();
             idx1++) {
          for (int idx2 = 0; idx2 < normalOrderFromLen[neighID2].size();
               idx2++) {
            auto len1 =
                pathPartShortest[idxPathPartShortest - 1][neighID1][idx1]
                    .distance; // shortest dist from start to [target - 1][idx1]
            auto len2 = allDistances[idJ_m][idJ_p][neighID1][neighID2][idx1]
                                    [idx2]; // dist from [target - 1][idx1] to
                                            // [target][idx2]
...
double DOPN::tryTwoOpt(int idxFrom, int idxTo) {
  ...
  int i = idxStartSamples - 1;
  int j = idxEndSamples;
  std::vector<std::vector<GraphNode_AngNeigh>> normalOrderToSamples =
      samples[i]; // i
  std::vector<std::vector<GraphNode_AngNeigh>> reverseOrderToSamples =
      samples[i + 1]; // i+1
  std::vector<std::vector<GraphNode_AngNeigh>> reverseOrderFromSamples =
      samples[j]; // j
  std::vector<std::vector<GraphNode_AngNeigh>> normalOrderFromSamples =
      samples[j + 1]; // j+1
  int idI = samples[i][0][0].node.id;
  int idI_p = samples[i + 1][0][0].node.id;
  int idJ = samples[j][0][0].node.id;
  ...
  // INFO("..... " <<" "<<idxStartSamples-1<<" "<<idxEndSamples<<" .....
  // "<<idxStartSamples <<" "<<idxEndSamples + 1<<" .....")
  // INFO("shortest.size()
  // "<<shortest.size()); INFO("shortest_back.size() "<<shortest_back.size());
  dop_2d_matrix_triple normalOrderTotLen =
      shortest[i]; // need to use idxStart+1 as to the
  ...
  int idxPathPartShortest = 1;
  // now calc shortest to reverseOrderFromSamples using distToReverseStart
  // INFO("calc shortest from "<<normalOrderToSamples[0].node.id <<" to "<<
  // reverseOrderFromSamples[0].node.id);
  // pathPartShortest[idxPathPartShortest].assign(reverseOrderFromSamples.size(),
  // dst_pair(-1, M));
  ...
  for (int neighID = 0; neighID < neighborhood_resolution; ++neighID) {
    pathPartShortest[idxPathPartShortest][neighID].assign(
        reverseOrderFromSamples[neighID].size(), NeighAngDist(-1, -1, M));
  }
  ...
  for (int target = idxEndSamples - 1; target >= idxStartSamples; target--) {
    ...
    for (int neighID = 0; neighID < neighborhood_resolution; ++neighID) {
      pathPartShortest[idxPathPartShortest][neighID].assign(
          samples[target][neighID].size(), NeighAngDist(-1, -1, M));
    }
    int idFrom = target + 1;
    int idTo = target;
...
void DOPN::twoOpt(int idxFrom, int idxTo) {
  ...
  // INFO("new two opt order is:")
  // INFO("..... " <<" "<<idxStartSamples-1<<" "<<idxEndSamples<<" .....
  // "<<idxStartSamples <<" "<<idxEndSamples + 1<<" .....") exit when dist
  // idxStart to idxEnd < 2 ---- need minimally distance of two*
  // reverse path between idx1+1 and idx2, so need
  if (idxTo - idxFrom < 2) {
    INFO_RED("idxEnd and idxStart are too close " << (idxTo - idxFrom)
                                                  << " must be lower than 2");
  ...
  if (idxTo >= targets.size()) {
    INFO_RED("idxTo is out of samples array " << idxTo << " samples number "
                                              << targets.size());
    exit(1);
  }
...
void DOPN::update() {
  ...
  /*
   //oldcalc of samples
   generateSamples(samples[idx++], start, resolution, neighborhood_radius,
   neighborhood_resolution, null_start_goal_radius, start.id, end.id); for (auto
   & t : targets) { generateSamples(samples[idx++], t, resolution,
   neighborhood_radius, neighborhood_resolution, null_start_goal_radius,
   start.id, end.id);
   }
   generateSamples(samples[idx], end, resolution, neighborhood_radius,
   neighborhood_resolution, null_start_goal_radius, start.id, end.id);
   */
  ...
  for (int target = 1; target < S + 1; target++) {
    ...
    for (int neighID = 0; neighID < samples[target].size(); ++neighID) {
      shortest[target][neighID].assign(samples[target][neighID].size(),
                                       NeighAngDist(-1, -1, M));
    }
    for (int fromNeighID = 0; fromNeighID < samples[target - 1].size();
         ++fromNeighID) {
      ...
      for (int toNeighID = 0; toNeighID < samples[target].size(); ++toNeighID) {
        ...
        for (int idx1 = 0; idx1 < S1; idx1++) {
          for (int idx2 = 0; idx2 < S2; idx2++) {
            ...
            auto len2 =
                allDistances[fromStates[0].node.id][toSamples[0].node.id]
                            [fromNeighID][toNeighID][idx1][idx2];
            auto nLen = len1 + len2;
  ...
  for (int neighID = 0; neighID < shortest_back[S].size(); ++neighID) {
    shortest_back[S][neighID].assign(samples[S][neighID].size(),
                                     NeighAngDist());
  }
  ...
  for (int target = S - 1; target >= 0; target--) {
    ...
    for (int neighID = 0; neighID < shortest_back[target].size(); ++neighID) {
      shortest_back[target][neighID].assign(samples[target][neighID].size(),
                                            NeighAngDist(-1, -1, M));
    }
    for (int fromNeighID = 0; fromNeighID < samples[target].size();
         ++fromNeighID) {
      auto &fromStates = samples[target][fromNeighID];
      const int S1 = fromStates.size();
      for (int toNeighID = 0; toNeighID < samples[target + 1].size();
           ++toNeighID) {
...
/*
 ...
 DOPN::allDistancesUsage[nodeFromID][nodeToID][neighFromID][neighToID][angFromID][angToID]
 += 1;
 }
 }
 */
...
void DOPN::updateAfterInsert(int idxStart, int idxEnd) {
  ...
  /*
   ...
   for (int var = idxStart; var <= idxEnd; ++var) {
   generateSamples(insertedSamples[var - idxStart], targets[var], resolution,
   neighborhood_radius, neighborhood_resolution, null_start_goal_radius,
   start.id, end.id);
   }
   */
  // INFO("ids before:");
  // this->listIds();
  samples.insert(samples.begin() + idxStart + 1, insertedSamples.begin(),
                 insertedSamples.end());
  // INFO("ids after:");
  // this->listIds();
  //	INFO("shortest insert ");
  shortest.insert(shortest.begin() + idxStart + 1, idxEnd - idxStart + 1,
                  dop_2d_matrix_triple());
  ...
  shortest_back.insert(shortest_back.begin() + idxStart + 1,
                       idxEnd - idxStart + 1, dop_2d_matrix_triple());
  ...
  for (int target = idxEnd + 1; target >= 0; target--) {
    ...
    for (int neighID = 0; neighID < shortest_back[target].size(); ++neighID) {
      shortest_back[target][neighID].assign(samples[target][neighID].size(),
                                            NeighAngDist(-1, -1, M));
      // INFO("resize inside shortest_back  to
      // "<<samples[target][neighID].size());
    }
...
void DOPN::updateAfterRemove(int idxStart, int idxEnd) {
  ...
  // INFO("shortest erase ");
  shortest.erase(shortest.begin() + idxStart + 1,
                 shortest.begin() + idxEnd + 2);
      ...
      for (int toNeighID = 0; toNeighID < samples[target].size(); ++toNeighID) {
        ...
        for (int idx1 = 0; idx1 < S1; idx1++) {
          for (int idx2 = 0; idx2 < S2; idx2++) {
            ...
            auto len2 =
                allDistances[fromID][toID][fromNeighID][toNeighID][idx1][idx2];
            // INFO(idx1<<" "<<idx2<<" len "<<len1<<" dist "<<distances[target -
            // 1][idx1][idx2]);
            auto nLen = len1 + len2;
            auto &len = shortest[target][toNeighID][idx2];
  ...
  shortest_back.erase(shortest_back.begin() + idxStart + 1,
                      shortest_back.begin() + idxEnd + 2);
...
std::vector<Dubins> DOPN::getPath() const {
  ...
  while (target > 0) {
    std::vector<std::vector<GraphNode_AngNeigh>> fromStates =
        samples[target - 1];
...
int DOPN::getNumTargets() { return targets.size(); }
...
std::vector<GraphNode> DOPN::getTargets(int idxFrom, int idxTo) {
  return std::vector<GraphNode>(targets.begin() + idxFrom,
                                targets.begin() + idxTo + 1);
}
...
int DOPN::getNumSamples() { return samples.size(); }
...
std::vector<GraphNode>::iterator DOPN::targetsEnd() { return targets.end(); }
...
/*
 std::vector<State> DOPN::getPathSampled(double initialSpeed, double
 maximal_speed, double maximal_acceleration, double time_sample_rate, double
 finalSpeed) { std::vector<State> path; std::vector<Dubins> dubinsPath =
 getPath(); INFO("getPathSampled we have "<<dubinsPath.size()<<" dubins paths");
 ...
 double distanceLeft = wholePathLength - (actualDistanceWhole + actualSpeed *
 time_sample_rate); double finalVelInSec = (actualSpeed - finalSpeed) /
 maximal_acceleration; double distanceToStop = 0.5 * (-maximal_acceleration) *
 finalVelInSec * finalVelInSec + actualSpeed * finalVelInSec; if (!deceleration
 && distanceLeft < distanceToStop) {
 ...
 deceleration = true;
 decelerationAcc = -(distanceLeft - (actualSpeed * finalVelInSec)) / (0.5 *
 finalVelInSec * finalVelInSec);
 ...
 distanceLeft = wholePathLength - (actualDistanceWhole);
 decelerationAcc = -(distanceLeft - (actualSpeed * finalVelInSec)) / (0.5 *
 finalVelInSec * finalVelInSec); if (decelerationAcc > maximal_acceleration) {
