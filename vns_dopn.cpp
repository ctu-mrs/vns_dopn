...
VNSDOPN::VNSDOPN(imr::CConfig& config, const std::string& problemFile) :
	...
	for (int var = 0; var < nodesAll.size(); ++var) {
		rewardAll += nodesAll[var].reward;
	}
	INFO_GREEN("rewardAll "<<rewardAll);
...
//DEBUG("SAVE_TIME_CPU: " << tSave.cpuTime());
	...
	/*
	 ...
	 config.add<std::string>("path", "file to save sampled path", "sampled-path.txt");
...
void VNSDOPN::fillResultRecord(int numIters, double length, int numItersLastImprovement, long timeLastImprovement) {
	...
	resultLog << result::newrec << name << getMethod() << t[0] << numIters << this->budget << radius << resolution << neighborhood_radius << neighborhood_resolution << final_reward << final_length
			<< numItersLastImprovement << timeLastImprovement << tourNodes.str();
