...
class VNSDOPN : public crl::CAlgorithm {
  typedef crl::CAlgorithm Base;
  typedef std::vector<int> IntVector;

public:
  VNSDOPN(crl::CConfig &config, const std::string &problemFile);
  virtual ~VNSDOPN();
  // Tour find(DatasetOP dataset);
  // Tour find(std::vector<GraphNode> nodes, double budget, int startIndex_, int
  // goalIndex_);

  static int
  greaterThanDistanceStartGoal(GraphNode gn1, GraphNode gn2,
                               struct SortCompareVar<GraphNode> data);
  static int greaterThanGraphNodeReward(GraphNode gn1, GraphNode gn2,
                                        struct SortCompareVar<GraphNode> data);
  // CAlgorithm functions
  void solve(void);
  std::string getRevision(void);
  static crl::CConfig &getConfig(crl::CConfig &config);
  static std::string getName(void) { return "vns_dopn"; }
...
protected:
  ...
  void fillResultRecord(int numIters, double length,
                        int numItersLastImprovement, long timeLastImprovement);
...
private:
  ...
  bool randomLocalSearch(DOPN &actualDOP, std::vector<GraphNode> &actualVNS,
                         int k);
  ...
  void generateInitialSolution(DOPN &actualDOP,
                               std::vector<GraphNode> &actualVNS);

  void largestRewardInsertion(DOPN &actualDOP,
                              std::vector<GraphNode> &actualVNS);

  bool insertRandom(DOPN &actualDOP, std::vector<GraphNode> &actualVNS,
                    int num_changes);
  bool exchangeRandom(DOPN &actualDOP, std::vector<GraphNode> &actualVNS,
                      int num_changes);
  NeighImrpovementValue
  improveNeighLocations(DOPN &actualDOPN, double minimal_improvement_distance);
  bool insertNeighborhoods(DOPN &actualDOPN,
                           std::vector<NeighAngValuesIds> originalNeighAngIds,
                           std::vector<bool> improvedNode,
                           std::vector<double> actualNeighAngles,
                           std::vector<GraphNode> actualGraphNodes);
  bool improveHeadings(DOPN &actualDOPN, std::vector<GraphNode> &actualVNS,
                       double minimal_improvement_distance);

  bool insertSystematic(DOPN &actualDOP, std::vector<GraphNode> &actualVNS,
                        int k);
  bool exchangeSystematic(DOPN &actualDOP, std::vector<GraphNode> &actualVNS,
                          int k);
