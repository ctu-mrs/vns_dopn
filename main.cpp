...
bool parseArgs(int argc, char *argv[]) {
	...
	desc.add_options()("help,h", "produce help message")("config,c", po::value<std::string>(&configFile)->default_value(std::string(argv[0]) + ".cfg"),
			"configuration file")("logger-config,l", po::value<std::string>(&loggerCfg)->default_value(loggerCfg), "logger configuration file")("config-gui",
			po::value<std::string>(&guiConfigFile)->default_value(std::string(argv[0]) + "-gui.cfg"), "dedicated gui configuration file")("problem",
			po::value<std::string>(&problemFile), "problem file");
	try {
		...
		if (vm.count("help")) {
			std::cerr << std::endl;
			std::cerr << "VNS Metaheuristic for Orienteering Problem with Neighborhoods " << FP_OP_VERSION << std::endl;
			std::cerr << cmdline_options << std::endl;
			ret = false;
		}
		if (ret && loggerCfg != "" && fs::exists(fs::path(loggerCfg))) {
			imr::initLogger("dopn", loggerCfg.c_str());
		} else {
			imr::initLogger("dopn");
		}
...
CoordsVector &load_goals_coords(const std::string &filename, CoordsVector &pts) {
	...
	for (int var = 0; var < loadedDataset.graph.size(); ++var) {
		//std::cout << "neighborhood-radius for load goal coords:" << neighborhood_radius << std::endl;
		pts.push_back(Coords(loadedDataset.graph[var].x + neighborhood_radius, loadedDataset.graph[var].y));
		pts.push_back(Coords(loadedDataset.graph[var].x - neighborhood_radius, loadedDataset.graph[var].y));
		pts.push_back(Coords(loadedDataset.graph[var].x, loadedDataset.graph[var].y + neighborhood_radius));
		pts.push_back(Coords(loadedDataset.graph[var].x, loadedDataset.graph[var].y - neighborhood_radius));
	}
	return pts;
}
